...
In the more general case, it's worth asking whether you _can_ implement fingerprint based analytics that don't use ePD-covered data.

The goal of the fingerprint is to emulate the token from classic analytics: that is, to attach an identifier to each user, so we can know if we've seen them before. This identifier doesn't have to be _exactly_ unique -- we might accept a risk of error in edge cases, e.g. multiple people on the exact same type of device from the exact same IP -- but we _do_ want it to be somewhat accurate.

Since we can't access any on-device data (e.g. canvas fingerprinting) without running afoul of the ePD, we'll have to make do with what's transmitted over the network.

If we were allowed to use the IP address, then we'd be essentially home free! While it is possible for multiple users to come from the same IP (either because they're from the same physical location, or because they're behind a VPN or NAT), we can generally consider it a _good enough_ approximation if we also disambiguate requests based on the `User-Agent` and other HTTP headers.

Since we aren't, we'd have to rely entirely on HTTP headers. Is this enough to uniquely identify individual users, at least down to a timescale of ~1 day (which appears to be a common interval for salt rotation in the privacy-aware analytics space)?

Surprisingly... maybe? Although it'd definitely require research with a larger sample size than 1 to say with any confidence, a quick look at HTTP header fingerprints ([try for yourself](https://amiunique.org/fingerprint) if you're interested) indicates that just my `User-Agent` is only seen in 0.09% of requests. Combine it with other non-traffic headers, like `Referer` or `Accept-Language`, and we might have a _good enough_ approximation.
